	...
	export default function SolutionCard({solution, liked, size="sm"}) {
		const [liking, setLiking] = useState(!liked);
		const [reply, setReply] = useState("");
		const [repliesNum, setRepliesNum] = useState(solution.replies_count);
		const errorRef = useRef(null);

		const { executeRecaptcha } = useGoogleReCaptcha();

		async function likeSolutionRequest(e) {
			errorRef.current.classList.add("hidden");

			const token = await executeRecaptcha("LIKESOLUTION");

			fetch(`/solution/${solution.id}/like`, {
				method: "POST",
				headers: {
		...
		async function submitReplyRequest(event) {
			...
			const token = await executeRecaptcha("REPLYSOLUTION");
		...
		function jumpToText(e) {
			e.preventDefault(); 
			sessionStorage.setItem("linkIDToFlash", "solution_link_" + solution.id); 
			window.location.href = "/viewer/" + solution.book_id;
		}

		return (
			<div className="solutioncard scroll-mt-36 p-6 border-y-2 border-gray-300 block gray-on-hover transition duration-200 rounded-lg mb-5">
				<div className={"solutionheader flex justify-between items-center " + (size === "sm" ? "mb-3" : "mb-5")}>
					<div className="useridentifiers flex items-center">
						<a href={solution.user ? "/profile/" + solution.user.id : "/"} className="mr-3 text-white bg-gray-200 w-10 h-10 rounded-full overflow-hidden">
						...
						<div>
							<h3 className="text-lg/[1rem] mt-2">{solution.title}</h3>
							<a href={solution.user ? "/profile/" + solution.user.id : "/"}>@{solution.user ? solution.user.username : "deleteduser"}</a>
						</div>
					</div>
					<div className="repliesetc apply-cursive">
						{Object.keys(solution.book.links).includes("solution_link_" + solution.id) ? 
								<button className="mr-3 text-indigo-700 underline" onClick={jumpToText}>jump to text</button> 
								: ""}
						<span>{repliesNum || "no"} reply(s)</span>
					</div>
				</div>
				<div className={"solutiontext " + (size === "lg" ? "mb-8" : "mb-4")}>
					{size === "sm" ? 
						trimToLength(solution.solution_text, 500) 
						:
						(
							<>
								<div className="higher-gray-hover transition duration-200 border-y-2 border-gray-300 mb-2 p-4 rounded-lg">
									<h3 className="text-lg">Problem text:</h3>
									{solution.problem_text}
								</div>
								<div className="higher-gray-hover transition duration-200 border-y-2 border-gray-300 p-4 rounded-lg">
									<h3 className="text-lg">Solution text:</h3>
				...
				<div className="interactions pr-10">
					...
					<div className="flex items-center">
						<button onClick={e => e.preventDefault()} className={!liking ? "green-icon" : ""}>
							<svg width="32" height="32" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={likeSolutionRequest}>
